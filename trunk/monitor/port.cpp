...
static RtlSetEnvironmentVariable_TYPE* GetRtlSetEnvironmentVariable() {
	...
	HMODULE zz = GetModuleHandleW(L"ntdll.dll");
					 ...
					 BOOL bTSEnabled, DWORD dwSessionId)
{
	...
	if (bTokOk)
	{
		...
		lpEnv = SetUserEnvironmentVariable(lpEnv, L"WPHF_totalpages", totalpages);
		lpEnv = SetUserEnvironmentVariable(lpEnv, L"WPHF_priority", priority);
		lpEnv = SetUserEnvironmentVariable(lpEnv, L"WPHF_submitted", submitted);
...
void CPort::Initialize()
{
	...
	*m_szOutputPath = L'\0';
	...
	m_pUserCommand = NULL;
	*m_szExecPath = L'\0';
	wcscpy_s(m_szGUIPath, LENGTHOF(m_szGUIPath), L"wphfgui.exe");
