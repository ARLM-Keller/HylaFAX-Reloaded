...
LPMONITOR2 WINAPI InitializePrintMonitor2(PMONITORINIT pMonitorInit, PHANDLE phMonitor)
{
	...
	*/
	...
	{
		g_pLog->Log(LOGLEVEL_ALL, L"WPHFMON is running on Windows XP or above");
...
#ifndef _DEBUG
	WCHAR env_dbg_raw[8];
	DWORD loglevel = LOGLEVEL_NONE, dwRet;
#endif
...
#else
		dwRet = GetEnvironmentVariableW(L"WPHF_DEBUG", env_dbg_raw, LENGTHOF(env_dbg_raw));
		if (dwRet > 0 && dwRet < LENGTHOF(env_dbg_raw))
		{
			loglevel = _wtoi(env_dbg_raw);
